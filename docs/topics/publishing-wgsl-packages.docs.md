...
2. **The same walk from the importing file's real path.** Under pnpm a package is a symlink into `node_modules/.pnpm`, and its own dependencies sit next to that store entry rather than next to the symlink; resolving the link first is what makes a WGSL package that imports another WGSL package work.
