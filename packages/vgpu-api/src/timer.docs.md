...
- A manual `gpu.frame()` that attached a span holds the timer's query set until you `submit()` it (a failed or abandoned frame releases it too). Dropping such a frame without submitting leaks those resources for the lifetime of the gpu — the same leak as a native `GPUCommandEncoder` you never `finish()` — because a frame is never assumed abandoned: it could still be submitted. Always submit the frames you open, or let `gpu.frame(cb)` do it for you; `gpu.dispose()` (or device loss) is the backstop.
