...
// Cloud/plasma look: stack octaves of inverted nearest-distance. There is no
// Perlin/simplex/fBM primitive in wgsl-std, so build the octaves yourself.
...
- **There is no Perlin, simplex, fBM, or value-noise primitive here** — those are deliberately deferred pending API and provenance review. Voronoi (cellular) noise is the only primitive, and it looks like cells, not smooth clouds. For a cloud/plasma/fog look, sum octaves of `1.0 - sample.f1` and warp the domain, as the third example above shows; for an animated version pass time as the `vec3f` Z coordinate. Validate the result with pixels (`npx vgpu check`, then a headless render), not by eye.
