...
test("keeps Darwin examples cache in memory only",async()=>{const root=await mkdtemp(join(tmpdir(),"cache-memory-"));cleanup.push(()=>rm(root,{recursive:true,force:true}));const cache=new ExamplesCache(join(root,"unused"),{platform:"darwin"}),path=cache.discoveryPath();await cache.write(path,Buffer.from("cached"));expect((await cache.read(path))?.toString()).toBe("cached");await expect(readFile(path)).rejects.toMatchObject({code:"ENOENT"});await cache.clear();expect(await cache.read(path)).toBeUndefined()});
