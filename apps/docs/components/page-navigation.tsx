...
export function PageNavigation({ prev, next, docs = false }: PageNavigationProps) {
  ...
  return (
    <nav aria-label="Previous and next pages" className="mt-14 grid gap-4 border-t border-gray-4 pt-6 md:grid-cols-2">
      {prev ? (
        <Link href={docs ? docsHref(prev.href) : prev.href} className="group rounded-lg border border-gray-4 bg-gray-1 p-4 transition-colors hover:border-gray-5 hover:bg-gray-2/50">
      ...
      {next ? (
        <Link href={docs ? docsHref(next.href) : next.href} className="group rounded-lg border border-gray-4 bg-gray-1 p-4 text-right transition-colors hover:border-gray-5 hover:bg-gray-2/50">
