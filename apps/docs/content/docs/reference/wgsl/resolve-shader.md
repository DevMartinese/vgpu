...
- **ESM-only:** the `./runtime` subpath declares only an `import` condition (no `require`) in `@vgpu/wgsl`'s `exports` map, unlike `loader-webpack` and `loader-vite`. Call `resolveShader()` from an ES module: a `.mjs`/`.mts` script, or a project with `"type": "module"` in `package.json`. From a CommonJS entry point — `npx tsx script.ts` in a project without `type: module`, for example — Node fails with `ERR_PACKAGE_PATH_NOT_EXPORTED`, not a `VGPU-*` error code. See the [no-bundler guide](/guides/no-bundler.docs.md) for the full script-from-disk workflow.
